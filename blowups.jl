...
print(test_gen == gens(c_2_simplified_ideal)[1]);  # Shows that the second chart has an A1 singularity after this coordinate change
...
X_fiber = spec(C, reduced_fiber_origin_ideal);
components_fiber = irreducible_components(X_fiber);

X_nonred_fiber = spec(C, fiber_origin_ideal);