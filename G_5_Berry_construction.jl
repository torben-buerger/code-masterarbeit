...
# We determine a smaller generating set for the preimage in S_quo
pre_blowup_basis = standard_basis(pre_blowup_ideal, ordering=negdegrevlex(S));
quo_blowup_basis = [simplify(quo_map(b)) for b in pre_blowup_basis];
test_quo_blowup_ideal = ideal(S_quo, quo_blowup_basis[[1:6; 8:9; 12; 15; 17; 20; 22; 26; 28]]);
print(test_quo_blowup_ideal == quo_blowup_ideal);  # Returns true, so the preimage ideal is generated by 15 elements of S_quo

# Compute the preimage of the ideal found by intersecting the ideal of vanishing semi invariants
...
# Next we compute the Rees algebra corresponding to the blowup in the blowup_ideal which is the homogeneous coordinate ring of the blown up space. To do so, we need an auxiliary variable t
Bt, y, t = polynomial_ring(K, :y => (1:12), :t => (1:1));

# Next we embed the relation ideal and the genretors for the ideal in which we will compute the blowup into the ring Bt
lift_S_Bt = hom(S, Bt, [y[i] for i in 1:12]);
relations_ideal_Bt = lift_S_Bt(relations_ideal);
blowup_vector = [2*y[1]^4 + 3*y[6],
    -y[1]^3*y[2] + (-2*zeta^4 + 1)*y[1]^2*y[5] + (-2*zeta^4 + 1)*y[2]*y[3] + 3*y[9],
    y[10],
    y[1]^3*y[4] + (-2*zeta^4 + 1)*y[1]^2*y[7] + (-2*zeta^4 + 1)*y[3]*y[4] + 3*y[11],
    y[1]^2*y[5] + y[2]*y[3],
    y[2]*y[4] + 8*y[3]^2,
    y[1]^2*y[7] + y[3]*y[4],
    y[1]*y[8] + y[2]*y[5],
    ...
    -y[1]*y[12] + y[4]*y[7],
    y[1]^2*y[4]^2 + 3*y[7]^2,
    -y[1]*y[2]^3 + (2*zeta^4 - 1)*y[1]*y[2]*y[8] + (2*zeta^4 - 1)*y[2]^2*y[5] + 3*y[5]*y[8],
    y[2]^4 + 3*y[8]^2,
    y[1]*y[4]^3 + (2*zeta^4 - 1)*y[1]*y[4]*y[12] + (-2*zeta^4 + 1)*y[4]^2*y[7] + 3*y[7]*y[12],
    y[4]^4 + 3*y[12]^2];
blowup_vector_t = [b*t[1] for b in blowup_vector];
blowup_vector_map = union([y[i] for i in 1:12], blowup_vector_t);

# Define the ring C=B[b_1,...,b_15] used to compute the rees algebra of the blowup, which can be embedded into this relative projective space as the ideal along which we blow up is generated by 15 elements
C, y, b = polynomial_ring(K, :y => (1:12), :b => (1:15));
blowup_map = hom(C, Bt, blowup_vector_map);
kernel_blowup_map = preimage(blowup_map, relations_ideal_Bt);
