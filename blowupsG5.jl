...
Aux_large, y, t, b = polynomial_ring(M, :y => (1:12), :t => (1:1), :b => (1:5));
embed_B = hom(Bt, Aux_large, union([y[i] for i in 1:12], [t[1]]));
embed_C = hom(C, Aux_large, union([y[i] for i in 1:12], [b[i] for i in 1:5]));
...
blowup_ideal_11 = ideal(Aux_large, union([embed_B(rel) for rel in gens(relations_ideal)], [b[i] - embed_B(blowup_vector_11[12+i])*t[1] for i in 1:5]));
